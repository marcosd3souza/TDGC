...
def train(X, A, n_clusters):
    ...
    # Laplacian Smoothing
    adj_norm_s = utils.preprocess_graph(adj, args.t, norm='sym', renorm=True)
    ...
    for seed in range(1):
        ...
        # init
        predict_labels, dis= utils.clustering(smooth_fea, n_clusters)#args.cluster_num)
    ...
    predict_labels, _ = utils.clustering(hidden_emb, n_clusters)
    hidden_emb = hidden_emb.detach().numpy()
    return hidden_emb, predict_labels
