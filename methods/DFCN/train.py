...
def Train(epoch, model, data, adj, lr, n_clusters, gamma_value, lambda_value, device):
    ...
    kmeans.fit(z_tilde.data.cpu().numpy())
    ...
    embedding = z_tilde.data.cpu().numpy()
    pred = KMeans(n_clusters=n_clusters, n_init=20).fit_predict(embedding)
