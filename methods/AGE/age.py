...
def _get_args():
    ...
    parser.add_argument('--dims', type=int, default=[100], help='Number of units in hidden layer 1.')
...
def gae_for(X, A, n_clusters):
    ...
    adj_norm_s = utils.preprocess_graph(adj, args.gnnlayers, norm='sym', renorm=True)
    ...
    pos_inds_cpu = torch.LongTensor(pos_inds).to(device)#.cuda()
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        while ( ed <= length ):
            sampled_neg = torch.LongTensor(np.random.choice(neg_inds, size=ed-st)).to(device)#.cuda()
            sampled_inds = torch.cat((pos_inds_cpu[st:ed], sampled_neg), 0)
            t = time.time()
            optimizer.zero_grad()
            ...
            zx = model(x)
            zy = model(y)
            batch_label = torch.cat((torch.ones(ed-st), torch.zeros(ed-st))).to(device)#.cuda()
    ...
    f_adj = np.matmul(hidden_emb, np.transpose(hidden_emb))
    pred = SpectralClustering(n_clusters, affinity='precomputed').fit_predict(f_adj)

    return hidden_emb, pred
